...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        ...
        return new Promise((resolve) => {
            resolve(`${address}:${self._getCurrentTimeStamp()}:starRegistry`)
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(time+60*5>currentTime &&  bitcoinMessage.verify(message, address, signature)){
                const data = {
                    address,
                    message,
                    signature,
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            const stars = self.chain.filter(block => {
                const uncodedBodyBlock = block.getBData()
                return uncodedBodyBlock.address === address
            })
            resolve(stars)
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach( (block, index) =>{
                ...
                if(index > 0){
                    if(self.chain[index].previousBlockHash === self.chain[index-1]){
